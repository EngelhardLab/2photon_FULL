...
%
% Trial data is kept in blocks, where every block stores the index to the
...
%                             , 'Licks'    ...
  ...
  methods
    ...
    function obj = ExperimentLog(vr, cfg, console, preallocSize)
      ...
      obj.savePerNTrials      = cfg.savePerNTrials;
      obj.newBlocks           = [];
      obj.writeCounter        = 0;
      obj.doComment           = 1;
      obj.startComment        = '';
...
%       obj.trialInfo.Licks     = zeros(0, size(vr.LickData,2), 'single');
      ...
      obj.trialEnded          = 0;
      obj.newBlock(vr, cfg.totalTrials);
    ...
    function log = save(obj, compact, timeNow, plots)
...
%       end
    ...
    function logExtras(obj, vr, rewardFactor)
%         global curlickbuffer curlickbuffer_timestamps size_hardware_lickbuffer new_lickdata_ctr sync_localtime_licktime
...
%       end
      ...
      % If a comment request has been made, prompt for it
      if ~isnan(obj.COMMENT_AT(obj.doComment))
        commentIndex        = obj.writeIndex + obj.COMMENT_AT(obj.doComment);
        trialIndex          = commentIndex - obj.block(end).firstTrial + 1;
        input               = inputDialog ( 'User comment'                                  ...
                                          , sprintf ( 'Comment for trial %d of session %d:' ...
                                                    , trialIndex                            ...
                                                    , numel(obj.session)                    ...
                                                    )                                       ...
                                          );
         
        logText             = sprintf('Comment for trial %d:  ', trialIndex);
        if isempty(input)
          logText           = [logText '(( CANCELLED ))'];
        else
          obj.block(end).trial(commentIndex).comment  ...
                            = strjoin({obj.block(end).trial(commentIndex).comment, input}, '\n');
          logText           = [logText, input];
        end
        
        if ~isempty(obj.console)
          obj.console.log(logText);
        end

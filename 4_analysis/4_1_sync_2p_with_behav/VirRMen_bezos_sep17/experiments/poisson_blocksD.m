...
try
  ...
  switch vr.state           % ... take action depending on the simulation state
    ...
    case BehavioralState.SetupTrial
      ...
      vr.guideFlashOn = true;
      vr.timeSinceOn = 0;
      vr.rwClock = vr.trialEndPauseDur;
      vr.RwFlag = 0;
    ...
    case BehavioralState.DuringReward
      ...
      rewardMSec  = vr.rewardFactor * vr.rewardMSec;
      ...
      vr.timeSinceOn = vr.timeSinceOn+vr.dt;
      % make visual guides flash
      for iHint = 1:numel(vr.choiceCueNames)
          triangles         = vr.(vr.choiceCueNames{iHint})(vr.trialType,:);
          if vr.guideFlashOn == true && vr.timeSinceOn >= .2 % flash at 8hz
            vr.worlds{vr.currentWorld}.surface.visible(triangles) = false;
            vr.timeSinceOn = 0;
            vr.guideFlashOn = false; 
          elseif vr.guideFlashOn == false && vr.timeSinceOn >= .2 % flash at 4hz
            vr.worlds{vr.currentWorld}.surface.visible(triangles) = true;
            vr.timeSinceOn = 0;
            vr.guideFlashOn = true; 
          end
...
function vr = setupTrials(vr, shaping)
  ...
  [vr.mainMazeID, vr.mazeID, vr.warmupIndex, vr.prevPerformance]  ...
                        = getTrainingLevel(vr.mazes, vr.exper.userdata.trainee, vr.protocol, vr.numMazesInProtocol);
  ...
  if isempty(vr.exper.userdata.trainee.stimulusBank)
    ...
    vr                  = checkStimulusBank(vr, vr.targetNumTrials, vr.trialDuplication, vr.trialDispersion);
    
  % Load custom stimulus bank if provided
  else
    vr.stimulusBank     = vr.exper.userdata.trainee.stimulusBank;
    vr                  = checkStimulusBank(vr);
  end
...
function vr = checkStimulusBank(vr, varargin)
  ...
  % Configure stimuli if so requested
  if nargin > 1
    vr.poissonStimuli.setTrialMixing(varargin{:});
  ...
  for iMaze = 1:numel(vr.mazes)
    ...
    if vr.poissonStimuli.configure(lCue, stimParameters{:});
      errordlg( sprintf('Stimuli parameters not configured for maze %d. Use the generatePoissonStimuli() function to pre-generate stimuli.', vr.mazeID)   ...
              , 'Stimulus sequences not configured', 'modal'  ...
              );
      vr.experimentEnded    = true;
      return;

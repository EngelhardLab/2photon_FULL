function code = poisson_patches_to_linear_track
...
%we crossed where the arms are so we reached the end of the linear track
      elseif isPastCrossing(vr.cross_arms, vr.position)
            vr.choice       = Choice(1); %doesn't matter
            vr.state        = BehavioralState.ChoiceMade;
    ...
    case BehavioralState.ChoiceMade
      ...
      vr = judgeVRTrial(vr,true); %always success

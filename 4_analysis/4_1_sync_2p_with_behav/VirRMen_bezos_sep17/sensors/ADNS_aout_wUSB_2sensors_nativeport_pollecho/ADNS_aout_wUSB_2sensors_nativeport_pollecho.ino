...
unsigned long num_replies = 0;  // number of poll requests and replies (they are the same)

unsigned long t_last_poll = 0;
  // time of previous mouse polling event (ms since startup)
  // measured at beginning of loop()

char cmd[10];  // serial communications command buffer
...
void loop() {
 ...
 // --- handle usb serial communication ---
  ...
  if (SerialUSB.available()) {
    ...
    int nBytes = SerialUSB.readBytesUntil(terminator_char, cmd, 10);
    
    // if input is the request character
    if (nBytes > 1 && cmd[0] == request_char) {
      ++num_replies;

      cmd[nBytes] = 0;
      sprintf ( reply, "%ld%c%ld%c%ld%c%ld%c%lu%c%lu%c%s%c"
              , delta_x, separator_char
              , delta_y, separator_char
              , delta_x2, separator_char
              , delta_y2, separator_char
              , t_accumulated, separator_char
              , num_replies, separator_char
              , cmd+1, terminator_char
              );

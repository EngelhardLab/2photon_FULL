...
char cmd[10];  // serial communications command
char reply[1000];  // serial communications reply buffer
long num_replies = 0;  // number of poll requests and replies (they are the same)
...
void loop() {
 ...
 // --- handle usb serial communication ---
  ...
  if (Serial.available()) {
    ...
    int nBytes = Serial.readBytesUntil(terminator_char, cmd, 10);
    ...
    if (nBytes > 1 && cmd[0] == request_char) {
      ...
      cmd[nBytes] = 0;
      sprintf ( reply, "%d%c%d%c%d%c%d%c%d%c%s%c"

...
function [velocity, rawData] = moveArduinoLinearVelocityMEX_simIdeal(vr)
  ...
  persistent ViewAng Speed iSpeed Count Trial StemLength
...
%     Speed = [50 60 70 80];
  ...
  if vr.protocol.currentTrial ~= Trial && vr.protocol.currentTrial > 0
    ...
    StemLength = vr.lCue + vr.lMemory/2;
  end
  if numel(Speed) == 1
    if vr.mazeID < 3
      Speed = 30;
...
%     end
    ...
    switch vr.trialType
      case Choice.L
        velocity = [-0.3*Speed(iSpeed), 0.9*Speed(iSpeed), 0, 1];
      case Choice.R
        velocity = [0.3*Speed(iSpeed), 0.9*Speed(iSpeed), 0, -1];
    end
  end

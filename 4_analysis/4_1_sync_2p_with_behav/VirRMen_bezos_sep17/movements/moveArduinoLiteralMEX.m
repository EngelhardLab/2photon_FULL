...
%
function [velocity, rawData] = moveArduinoLiteralMEX(vr)
...
% persistent dX_vec dx_ctr
% window_size = 50;
% if isempty(dX_vec)
%    dX_vec = zeros(1,window_size);
% end
% if isempty(dx_ctr)
%    dx_ctr = 1;
% end
  ...
  dy1 = rotation_transform;
  rotmat = [cos(dy1) -sin(dy1);sin(dy1) cos(dy1)];
  newD = rotmat*[dX;dY];
%   dX=newD(1);
%   dY=newD(2);
...
%   dX=dX-mean(dx_ctr)/2;
  ...
  if dT == 0
    ...
    %
%     if rand>0.99
%         disp(num2str([velocity(:)' vr.orientation]))
%     end

...
function res=process_problem_patch2(output_folder,patch_file,patch_ctr,want_red_channel,use_red_channel,flag_downsample,good_chunks)
...
for chunk_ctr = 1:num_chunks
    ...
    if ~ismember(chunk_ctr,good_chunks)
        curbadchunk = -ones(imsize_extract+2*max_shift,imsize_extract+2*max_shift, num_frames_per_chunk(chunk_ctr) );
        mc_image_stack_full = cat(3,mc_image_stack_full,curbadchunk);

        if want_red_channel
            mc_image_stack_full_red = cat(3,mc_image_stack_full_red,curbadchunk);
        end
    else

...
for i=1:num_patches
    ...
    if ~exist(patch_taken_file,'file') && k == 0 
        ...
        dbstop in process_fast if error
        process_fast(i,output_folder,patch_file,i,want_red_channel,use_red_channel);
       
        patches_processed_end_times{end+1} = datetime;
        k = 1 ;
    end
    patch_done_file  = [output_folder,'Patch ',num2str(i),' taken_done.txt'];
    f = fopen( patch_done_file, 'w' );
    fclose(f);
end
...
disp('222222222222222222222222222222222222222222 Finished all patches')

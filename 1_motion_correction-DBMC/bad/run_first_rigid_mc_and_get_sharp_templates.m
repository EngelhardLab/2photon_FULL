...
chunks_green_filenames = cell(num_chunks,1);
chunks_red_filenames   = cell(num_chunks,1);
...
for i=1:num_chunks
    ...
    %also make sharp template here
    cur_savefilename=[sharptemplate_folder,'template_',num2str(i),'.mat'];
    if exist(cur_savefilename,'file')
        continue
    end

    %ImageStack_mc=apply_mc_gpu(stack,YY_cell{i},XX_cell{i});
    ImageStack_mc=apply_mc(stack,YY_cell{i},XX_cell{i});
...
if have_red_channel && use_red_channel
    ...
    for i=1:num_chunks
        ...
        %mc_stack = apply_mc_gpu(stack,YY_cell{i},XX_cell{i});

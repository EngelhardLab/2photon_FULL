...
patches_folder = fullfile(output_folder, 'patches_folder', filesep);

% Check if the folder actually exists on the NAS
if ~exist(patches_folder, 'dir')
    error('CRITICAL ERROR: The folder "%s" was not found!', patches_folder);
end

% Look for .roi files (Try both lowercase and uppercase just in case)
patcheslist = [dir(fullfile(patches_folder, '*.roi')); dir(fullfile(patches_folder, '*.ROI'))];
num_patches = length(patcheslist);

% Print the count so you can see it in the terminal
fprintf('Found %d patches in %s\n', num_patches, patches_folder);

if num_patches == 0
    error('CRITICAL ERROR: No .roi files found in the patches folder!');
end

if input_folder(end) ~= '/' && input_folder(end) ~= '\'
    input_folder = [input_folder, filesep];
end
slashfind = strfind(input_folder, filesep);

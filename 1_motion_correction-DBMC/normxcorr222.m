function C = normxcorr222(varargin)
...
C = zeros(size(numerator),"gpuArray");%-drn
%avgs_gpu = zeros(size(stack,1),size(stack,2),floor(size(stack,3)/avg_win),"gpuArray");
...
%%
%-------------------------------------------
%  PREVIOUS FUNCTION USED BY normxcorr2.m 
%-------------------------------------------
% Function  local_sum
%
...
%B = padarray(A,[m n]); /// padding works with -normal- arrays 
%s = cumsum(B,1); /// cumsum is also taking longer than other funcs
...
%-------------------------------
% Function  local_sum
% based on local_sum from matlab's normxcorr2 function to use GPU arrays (gpuArray)

function local_sum_A = local_sum(A, m, n)

    % is A gpuArray ?
    if ~isa(A, 'gpuArray')
        error('Input array is not gpuArray.');
    end

    % simulating the padding function
    % padding function adds digits on rows and columns to 'pad' the array
    padValue = 0; % filling with zeros - we only need zeros i guess
    B = gpuPadArray(A, [m, n], padValue);

    % cumsum - not sure if done on the GPU (!!!!)
    s = cumsum(B, 1); % acc sum over rows [row1, row1+row2, row1+row2+row3, ...] 
    c = s(1 + m:end - 1, :) - s(1:end - m - 1, :); 

    s = cumsum(c, 2); 
    local_sum_A = s(:, 1 + n:end - 1) - s(:, 1:end - n - 1); 

function paddArray = gpuPadArray(gpu_arr, pad_dim, padwith)
    % function to simulate padarray(A) using gpuArray.   
    % padding function adds digits on rows and columns to 'pad' the array

    % inputArray- must be gpuArray type.
    % pad_dim- number of padding rows and cols [padRows, padCols].
    % padwith- value to fill the padding with > we just need zeros

    [rows, cols] = size(gpu_arr);

    % padding dimensions
    pad_rows = pad_dim(1);
    pad_cols = pad_dim(2);

    % first the padding is created as arrays that are then added
    % dims for top & bottom : [rows : #rows of pad, 
    %                          cols : #rows of array + 2 * #cols of pad]
    top_pad = gpuArray.ones(pad_rows, cols + 2 * pad_cols) * padwith;
    bottom_pad = gpuArray.ones(pad_rows, cols + 2 * pad_cols) * padwith;

    % dims for l & r : [rows : #rows of array, 
    %                   cols : #cols of pad]
    left_pad = gpuArray.ones(rows, pad_cols) * padwith;
    right_pad = gpuArray.ones(rows, pad_cols) * padwith;

    % joining the padding and gpuArray to get the padded matrix
    paddArray = [
        top_pad;                                 
        [left_pad, gpu_arr, right_pad];     
        bottom_pad                                
    ];
...
xcorr_ab = ifft22(Fa .* Fb,'symmetric');%drn
%-------------------------------
% Function  time_conv2
%

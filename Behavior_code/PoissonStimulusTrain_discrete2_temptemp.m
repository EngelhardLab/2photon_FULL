classdef PoissonStimulusTrain_discrete2_temptemp < handle
  ...
  methods
    ...
    %----- Constructor
    function obj = PoissonStimulusTrain_discrete2_temptemp(targetNTrials, trialDuplication, trialDispersion)
      ...
      obj.default.stim.cuePos       = cell(size(PoissonStimulusTrain_discrete2_temptemp.CHOICES));
      obj.default.stim.cueCombo     = nan(numel(PoissonStimulusTrain_discrete2_temptemp.CHOICES), 0);
...
%         end
        ...
        if ~isEdgeTrial %normal case
            ...
            while nCuesTotal > cfg.maxNumCues || nCuesTotal==0
                for iSide = 1:numel(meanNumCues)
                    % Draw a Poisson count of towers within available length
                    nCues(iSide)        = poissrnd(meanNumCues(iSide));
                end
            ...
            possible_diff_cues_inds = x_skell_edge_ind:find(x_skell==cfg.maxNumCues);                    
            ...
            possible_nCues1 = diff_cCues:cfg.maxNumCues;
            cond_pdf = poisspdf(possible_nCues1 ,meanNumCues(1)).*poisspdf(0:(cfg.maxNumCues-diff_cCues),meanNumCues(2));
...
%                   ;
  ...
  methods (Static)
    ...
    function obj = loadobj(frozen)
      ...
      obj               = PoissonStimulusTrain_discrete2_temptemp();
